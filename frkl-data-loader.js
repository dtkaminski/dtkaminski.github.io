...
(function () {
  ...
  async function refresh() {
    ...
    try {
      ...
      // ── Patterns (already live — they're generated by oi_export_patterns.py and stored in Supabase) ──
      if (patterns && patterns.patterns.length > 0) {
        const prior = window.FRKL_PATTERNS || {};
        window.FRKL_PATTERNS = {
          ...prior,
          patterns: patterns.patterns,
          diff: patterns.diff || prior.diff,
          money_rollup: patterns.money_rollup || prior.money_rollup,
          money_patterns: patterns.money_patterns || prior.money_patterns,
          action_money: patterns.action_money || prior.action_money,
          action_phit: patterns.action_phit || prior.action_phit,
          actions_with_attribution: patterns.actions_with_attribution || prior.actions_with_attribution,
          scorecard: patterns.scorecard || prior.scorecard,
          diagnoses_by_parent: patterns.diagnoses_by_parent || prior.diagnoses_by_parent,
          metrics: patterns.metrics || prior.metrics,
      ...
      if (actions && Object.keys(actions).length > 0) {
        window.FRKL_ACTION_STATUS = { ...(window.FRKL_ACTION_STATUS || {}), ...actions };
      }

      // ── Daily channel data — only override if Supabase has it ──
      if (window.FRKL_DATA) {
        if (dailyShopify.length > 0) window.FRKL_DATA.shopify = dailyShopify;
        if (dailyMeta.length > 0) window.FRKL_DATA.metaDaily = dailyMeta;
        if (dailyGoogle.length > 0) window.FRKL_DATA.googleAds = dailyGoogle;
        if (dailyGa4.length > 0) window.FRKL_DATA.ga4 = dailyGa4;
        if (klaviyoDaily.length > 0) window.FRKL_DATA.klaviyo = klaviyoDaily;
